...
DOCUMENTATION = '''
---
module: disk_stress_pod

short_description: A module that stress disk pods

version_added: "2.8"

description:
    - "A module that stress disk pods."
...
def inyect_disk(name, namespace, module):
    ...
    exec_command = [
        '/bin/sh',
        '-c',
        'apt update; apt-get install -y stress-ng; stress-ng --hdd 1 --aggressive --timeout 2m',
        ]
...
def run_module():
    ...
    for experiment in counts:
        ...
        if (int(experiment) > len(pod_list)):
            to_be_disk = random.sample(pod_list, len(pod_list))
        else:
            to_be_disk = random.sample(pod_list, int(experiment))

        for pod in to_be_disk:
            inyect_disk(pod.metadata.name, pod.metadata.namespace, module)
